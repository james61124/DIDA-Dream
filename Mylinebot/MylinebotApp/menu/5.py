body = {
    "size": {"width": 1200, "height": 810},
    "selected": "true",
    "name": "Controller",
    "chatBarText": "Controller",
    "areas":[
        {
          "bounds": {"x": 0, "y": 0, "width": 400, "height": 93},
          "action": {"type": "message", "text": "header1"}
        },
        {
          "bounds": {"x": 400, "y": 0, "width": 400, "height": 93},
          "action": {"type": "message", "text": "header2"}
        },
        {
          "bounds": {"x": 800, "y": 0, "width": 400, "height": 93},
          "action": {"type": "uri",
                      "label": "MY DIDA",
                      "uri": 'https://liff.line.me/1657635549-Gllkgw3o'}
        },
        {
          "bounds": {"x": 0, "y": 93, "width": 800, "height": 335},
          "action": {"type": "message", "text": "upleft"}
        },
        {
          "bounds": {"x": 800, "y": 93, "width": 400, "height": 335},
          "action": {"type": "message", "text": "upright"}
        },
        {
          "bounds": {"x": 0, "y": 428, "width": 400, "height": 335},
          "action": {"type": "message", "text": "downleft"}
        },
        {
          "bounds": {"x": 400, "y": 428, "width": 400, "height": 335},
          "action": {"type": "message", "text": "downmiddle"}
        },
        {
          "bounds": {"x": 800, "y": 428, "width": 400, "height": 335},
          "action": {"type": "uri",
                      "label": "downright",
                      "uri": 'https://liff.line.me/1657635549-p87K6gk3'}
        }
        
    ]
}